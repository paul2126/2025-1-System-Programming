...
static Chunk_T allocate_more_memory(Chunk_T prev, size_t units) {
  ...
  if (g_free_head == NULL)
    ...
    // just return c, insert will be handled by caller
    return c;
  } else
...
/*--------------------------------------------------------------*/
/* get_bin_idx
 * Returns the bin index for given units
 */
/*--------------------------------------------------------------*/
static int get_bin_idx(size_t units) {
  int bin_idx = 0;
  if (units == 1)
    bin_idx = 0;
  else if (units <= 2)
...
/*--------------------------------------------------------------*/
/* find_free_chunk
 * Returns the first free chunk with >units in the bin_idx and upwards
 */
/*--------------------------------------------------------------*/
static Chunk_T find_free_chunk(size_t units) {
  Chunk_T c = NULL;
  for (int i = get_bin_idx(units); i < 12; i++) {
    if (bins[i] != NULL) {
      for (c = bins[i]; c != NULL; c = chunk_get_next_free_chunk(c)) {
        if (chunk_get_units(c) >= units + 5 ||
            chunk_get_units(c) == units + 2)
          // +5 is for remaining chunk (at least 3 units)
          // +2 is for header & footer
          return c;
      }
    }
  }
...
/*--------------------------------------------------------------*/
/* insert_into_bin
 * Insert given Chunk into the bin_idx head
 */
/*--------------------------------------------------------------*/
static void insert_into_bin(Chunk_T c, int bin_idx) {
  chunk_set_status(c, CHUNK_FREE);
  if (bins[bin_idx] == NULL) { // empty bin
    bins[bin_idx] = c;
    chunk_set_next_free_chunk(c, NULL);
  } else { // not empty bin
    Chunk_T pos;

    // find the place for c
    for (pos = bins[bin_idx]; pos != NULL;
         pos = chunk_get_next_free_chunk(pos)) {
      // if pos is the first chunk
      if (c < pos && chunk_get_next_free_chunk(pos) == NULL) {
        chunk_set_next_free_chunk(c, pos);
        chunk_set_prev_free_chunk(c, NULL);
        chunk_set_prev_free_chunk(pos, c);
        bins[bin_idx] = c;
        break;
      } else if (c > pos && chunk_get_next_free_chunk(pos) == NULL) {
        chunk_set_next_free_chunk(pos, c);
        chunk_set_prev_free_chunk(c, pos);
        chunk_set_next_free_chunk(c, NULL);
        break;
      }
      // if c position is in the middle of the list
      if (c < chunk_get_next_free_chunk(pos)) {
        chunk_set_next_free_chunk(pos, c);
        chunk_set_prev_free_chunk(c, pos);
        chunk_set_next_free_chunk(c, chunk_get_next_free_chunk(pos));
        chunk_set_prev_free_chunk(chunk_get_next_free_chunk(pos), c);
        break;
      }
    }
  }
...
/*--------------------------------------------------------------*/
/* remove_from_bin
 * Remove given Chunk from the bin_idx
 */
/*--------------------------------------------------------------*/
static void remove_from_bin(Chunk_T c, int bin_idx) {
...
/*--------------------------------------------------------------*/
/* binning_coalesce:
 * Freed chunk will coalesce with adjacent chunks and insert into
 correct bin.
 merged chunks are removed from the bin
 */
/*--------------------------------------------------------------*/
static void binning_coalesce(Chunk_T prev, Chunk_T c) {
  ...
  if (next != NULL && chunk_get_status(next) == CHUNK_FREE) {
    ...
    remove_from_bin(next, get_bin_idx(chunk_get_units(next)));
    chunk_set_status(next, CHUNK_FREE);
  }

  remove_from_bin(c, get_bin_idx(chunk_get_units(c)));
  insert_into_bin(c, get_bin_idx(chunk_get_units(c)));
  chunk_set_status(c, CHUNK_FREE);
}
...
void *heapmgr_malloc(size_t size) {
  ...
  //   int bin_idx = get_bin_idx(units);

  pprev = NULL;
  prev = NULL;

  // first search in the bin
  c = find_free_chunk(units);
  if (c == NULL) { // if no match found
    c = allocate_more_memory(prev, units);
    chunk_set_status(c, CHUNK_FREE);

    // units of remaining free chunk
    int remaining_units = chunk_get_units(c) - units - 2;
    // assign free chunk to bin
    insert_into_bin(c, get_bin_idx(remaining_units));
  ...
  // initialize element if not initialized
  ...
  // traverse the free chunk list
  //   for (c = bins[bin_idx]; c != NULL; c =
  //   chunk_get_next_free_chunk(c)) {
  //     if (chunk_get_units(c) == units + 2) { // perfect fit
  //       remove_from_bin(c, bin_idx);         // use this chunk.
  //       remove it assert(check_heap_validity()); return (void
  //       *)((char *)c + CHUNK_UNIT);
  //     } else if (chunk_get_units(c) > units + 2) { // too big
  //       c = split_chunk(c, units);                 // split chunk
  //       assert(check_heap_validity());
  //       return (void *)((char *)c + CHUNK_UNIT);
...
void heapmgr_free(void *m) {
  Chunk_T c, prev;
  dump_heap_to_dot("before-free.dot");
  ...
  // get prev chunk
  if ((char *)c > (char *)g_heap_start + CHUNK_UNIT) { // notfirst chunk
    int prev_unit = *(int *)((char *)c - CHUNK_UNIT);
    prev = (Chunk_T)((char *)c - (prev_unit + 2) * CHUNK_UNIT);
  }
  binning_coalesce(prev, c);

  /* traverse the free chunk list to find the
     right location for 'c' */
  //   for (w = g_free_head; w != NULL; w =
  //   chunk_get_next_free_chunk(w)) {
  //     if (c < w)
  //       break;
  //     prev = w;
  //   }
...
static void dump_heap_to_dot(const char *filename) {
  ...
  // dump bins
  for (int i = 0; i < 11; i++) {
    fprintf(f, "  bin_%d [label=\"Bin %d\"];\n", i, i);
    Chunk_T b;
    for (b = bins[i]; b != NULL; b = chunk_get_next_free_chunk(b)) {
      // get chunk
      fprintf(f,
              "  chunk_%p [label=\"{Address: %p | Size: %d | Status: "
              "%s}\"];\n",
              (void *)b, (void *)b, chunk_get_units(b),
              chunk_get_status(b) == CHUNK_FREE ? "FREE" : "IN_USE");
      // link to bin
      fprintf(f,
              "  bin_%d -> chunk_%p [label=\"bin\""
              "color =\"red\"];\n",
              i, (void *)b);
      // link to next adjacent chunk
      //   Chunk_T next =
      //       chunk_get_next_adjacent(b, g_heap_start, g_heap_end);
      //   if (next != NULL) {
      //     fprintf(f, "  chunk_%p -> chunk_%p
      //     [label=\"adjacent\"];\n",
      //             (void *)b, (void *)next);
      //   } else {
      //     break;
